...
export default function NewProposalCRMPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [productsSnap, logosSnap, proposalsSnap, clientSnap] = await Promise.all([
          getDocs(query(collection(db, 'products'), where('orgId', '==', orgId))),
          getDocs(query(collection(db, 'logos'), where('orgId', '==', orgId))),
          getDocs(query(collection(db, 'proposals'), where('orgId', '==', orgId), orderBy('number', 'desc'), limit(1))),
        ...
        if (!proposalsSnap.empty) {
          const num = ((proposalsSnap.docs[0].data() as any).number || 0) + 1
          setNextNumber(num)
        }
        ...
        if (clientSnap?.exists()) {
          ...
          setValue('clientId', clientSnap.id)
        }
      } catch (error) {
