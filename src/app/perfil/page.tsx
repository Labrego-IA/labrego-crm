...
export default function PerfilPage() {
  ...
  const handleCancelPlan = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        toast.error(data.error || 'Erro ao cancelar o plano.')
        return
      }
      ...
      toast.success(
        cancelDate
          ? `Cancelamento solicitado. Seu plano permanece ativo ate ${cancelDate}.`
    ...
    <div className="space-y-6">
      ...
      {plan !== 'free' && member?.role === 'admin' && !isCanceling && (
        <div className="bg-white rounded-2xl border border-red-200/60 shadow-sm p-6">
          ...
          <p className="text-sm text-slate-500 mt-1 mb-4">
            Ao cancelar, seu plano permanece ativo ate o final do periodo de cobranca atual. Apos isso, sua conta sera convertida para o plano gratuito.
          </p>
          <button
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-6">
      ...
      {/* Cancel Plan Modal */}
      {!isPartnerView && <Modal isOpen={showCancelPlanModal} onClose={() => setShowCancelPlanModal(false)} size="md" centered>
        <div className="space-y-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Cancelar plano</h3>
              <p className="text-sm text-slate-500">Tem certeza que deseja cancelar?</p>
            </div>
          </div>

          <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
            <p className="text-sm text-amber-700">
              Seu plano <span className="font-semibold">{display.displayName}</span> permanecera ativo ate o final do periodo de cobranca atual.
              Apos esse periodo, sua conta sera convertida para o plano gratuito com funcionalidades limitadas.
            </p>
          </div>

          <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">O que voce perde ao cancelar:</p>
            <ul className="space-y-1.5 text-sm text-slate-600">
              <li className="flex items-center gap-2">
                ...
                Ate {limits.maxUsers} usuarios (plano gratuito: 1)
              </li>
              <li className="flex items-center gap-2">
                <svg className="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                Ate {limits.maxContacts.toLocaleString('pt-BR')} contatos
              </li>
              <li className="flex items-center gap-2">
                <svg className="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                {limits.monthlyMinutes.toLocaleString('pt-BR')} minutos mensais
              </li>
              <li className="flex items-center gap-2">
                <svg className="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                {limits.maxConcurrentAgents} agentes simultaneos
              </li>
            </ul>
          </div>
