    ...
    <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-6">
      ...
      {status === 'disconnected' && (
        <div className="text-center py-8">
          <p className="text-white/60 mb-4">Conecte seu WhatsApp para ativar o agente de atendimento.</p>
          <button
            onClick={handleConnect}
            disabled={loading}
            className="px-6 py-3 bg-green-600 hover:bg-green-500 text-white font-medium rounded-xl transition-colors disabled:opacity-50"
          >
      ...
      {(status === 'qr_ready' || status === 'connecting') && (
        <div className="text-center py-4">
          <p className="text-white/60 mb-4">Escaneie o QR Code com seu WhatsApp para conectar:</p>
          {qrCode ? (
            ...
            <div className="inline-flex items-center justify-center w-64 h-64 bg-slate-700/50 rounded-2xl mb-4">
              <div className="animate-spin w-8 h-8 border-2 border-[#13DEFC] border-t-transparent rounded-full" />
            </div>
          )}
          <p className="text-white/40 text-sm">Aguardando escaneamento...</p>
          <button
            onClick={handleDisconnect}
            className="mt-4 px-4 py-2 text-white/50 hover:text-white/80 text-sm transition-colors"
          >
            Cancelar
          </button>
        </div>
        ...
        <div className="py-4">
          <div className="flex items-center gap-3 p-4 bg-green-500/10 border border-green-500/30 rounded-xl mb-4">
            ...
            <div>
              ...
              {phoneNumber && <p className="text-white/50 text-sm">{phoneNumber}</p>}
            </div>
          </div>
          <button
            onClick={handleDisconnect}
            disabled={loading}
            className="px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-400 font-medium rounded-xl transition-colors text-sm disabled:opacity-50"
          >
...
function StatusBadge({ status }: { status: WhatsAppConnectionStatus }) {
  const config = {
    ...
    error: { label: 'Erro', bg: 'bg-red-500/20', text: 'text-red-400' },
  }
