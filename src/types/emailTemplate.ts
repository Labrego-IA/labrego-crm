...
export function createDefaultBlock(type: BlockType): EmailBlockData {
  ...
  switch (type) {
    ...
    case 'button':
      return { ...base, buttonText: 'Clique aqui', buttonUrl: 'https://', buttonColor: '#06B6D4', buttonTextColor: '#FFFFFF', buttonRadius: 6, align: 'center' }
    case 'divider':
...
function blockToRow(block: EmailBlockData): string {
  ...
  switch (block.type) {
    ...
    case 'button': {
      const btnColor = sanitizeColor(block.buttonColor || '') || '#06B6D4'
